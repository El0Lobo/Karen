...
document.addEventListener("DOMContentLoaded", () => {
  const entries = window.entries.sort((a, b) =>
  ...
  const audio = new Audio(
    "https://github.com/El0Lobo/Karen/raw/main/Stuff%20to%20host/batman-transition-download-sound-link.mp3"
  );
  ...
  const filterEntries = (filter) => {
    ...
    if (filter === "all") {
      ...
      filteredEntries = entries.filter(
        (entry) =>
          entry.toLowerCase().startsWith(filter) ||
          (!isNaN(entry[0]) && filter === "#")
      );
      displayEntries(filteredEntries);
    }
  };
